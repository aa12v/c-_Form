...
namespace Sistem
{
    public partial class Form10 : Form
    {
        ...
        {
            string ara = "SELECT * FROM kullanici WHERE isim LIKE '%' + @isim + '%' OR barcode = @barcode";
        ...
        private void button_kaydiBarkodaDonustur_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string tarihKod = saat + dakika + gun + ay + yil.Substring(yil.Length - 2);
                ...
                // İlk harfleri al ve oluşturulacak barkodu oluştur
                char ilkHarfIsim = isim[0];
                char ilkHarfSoyisim = soyisim[0];
                string olusturulacakBarcode = $"{ilkHarfIsim}{ilkHarfSoyisim}";

                // Barkodu TextBox'lara ve PictureBox'a yaz
                textBox_barcode.Text = "*" + "(XX)XXXXXXX" + tarihKod + "(" + olusturulacakBarcode + ")" + "*";
                textBox_olusturulacakBarcode.Text = "*9999999999999*";

                // Barkodu PictureBox'ta görüntüle
             /*   Zen.Barcode.Code128BarcodeDraw barcode = Zen.Barcode.BarcodeDrawFactory.Code128WithChecksum;
                pictureBox_barcode.Image = barcode.Draw(textBox_olusturulacakBarcode.Text, 160);*/

                // Veritabanına kaydet
                string kayit = "INSERT INTO kullanici (meslekiKimlik, isim, soyisim, telefonNo, eMailAdres, kullaniciAd, parola, rol, barcode) VALUES (@meslekiKimlik, @isim, @soyisim, @telefonNo, @eMailAdres, @kullaniciAd, @parola, @rol, @barcode)";
